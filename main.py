...
from config import (
    ...
    OWNER_ID, BOOSTED_FILENAME, TMP_DIR
)
...
async def convert_to_extreme_bass(input_path: str, output_path: str):
    """Apply extreme bass to a voice/audio file."""
...
async def owner_voice_handler(c: Client, m: Message):
    """Handles owner's voice messages."""
    fancy_msg = "🎵 **Voice received!** Preparing extreme bass…"
    await m.reply_text(fancy_msg, parse_mode="markdown")
    ...
    try:
        ...
        # Use executor to run CPU-heavy processing
        await asyncio.get_event_loop().run_in_executor(None, convert_to_extreme_bass, in_path, out_path)
        playing_state['file_path'] = out_path
        await m.reply_text(
            "🎧 **Extreme bass applied!**\n"
...
async def receive_chat_id(c: Client, m: Message):
    """Receives chat ID and joins VC to play the boosted audio."""
    chat_id = int(m.text.strip())
    if not playing_state.get('file_path'):
        return await m.reply_text("❌ No processed audio found.")

    await m.reply_text(f"🔊 **Joining VC of chat:** `{chat_id}`…", parse_mode="markdown")
    ...
    try:
        ...
        await m.reply_text("▶️ **Now playing with EXTREME BASS!** Use `/stopdj` to stop.", parse_mode="markdown")
    except Exception as e:
        logger.exception(e)
...
async def stop_handler(c: Client, m: Message):
    """Stops the bass playback."""
    if not playing_state['is_playing']:
        return await m.reply_text("⚠️ Nothing is playing right now.")
    try:
        chat_id = playing_state['chat_id']
        playing_state['is_playing'] = False
        if pytg:
            await pytg.leave_group_call(chat_id)
        await m.reply_text("⏹️ **Playback stopped.**", parse_mode="markdown")
    except Exception as e:
        logger.exception(e)
        await m.reply_text("⚠️ Could not stop playback.")
