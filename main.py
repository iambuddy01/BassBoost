...
in_path = await bot.download_media(m.voice.file_id, file_name=os.path.join(TMP_DIR, "incoming_voice.ogg"))
try:
out_path = BOOSTED_FILENAME
await asyncio.get_event_loop().run_in_executor(None, lambda: convert_to_extreme_bass(in_path, out_path))
playing_state['file_path'] = out_path
await m.reply_text("🎧 Done! Now send group chat id to play (-100...) or /cancel to abort.")
...
await m.reply_text(f"🔊 Joining `{chat_id}` VC...", parse_mode="markdown")
...
return await m.reply_text("⚠️ Nothing playing.")
try:
...
await bot.start(); await assistant.start()
logger.info("Bass bot started. Waiting for owner...")
